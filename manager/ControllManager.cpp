...
{
    QFile file("C:/Users/11749/AppData/Roaming/DSControl/deploy"); //+ userManualFilename_);
...
//     }
    ...
    {
        _serverComm->AssignDeployTask(std::get<0>(*it),std::get<1>(*it) , std::get<2>(*it));
    }
...
ControllManager::DeployList  ControllManager::createDeployFiles()
{
    ...
    {
        fileList.push_back(std::make_tuple(matchIP((dep.ip).toStdString()),(dep.appName).toStdString(),(dep.file).toStdString()));
...
/*
ControllManager::DeployList ControllManager::makeDeployFiles()
{
    QString appPath=QCoreApplication::applicationDirPath();
    QString xmlPath=appPath+"/";
    xmlPath+="deploytest.xml";
    qDebug() << xmlPath;
    QFile file(xmlPath);
    ...
    QDomNode task=root.firstChild();

    ControllManager::DeployList fileList;
    while (!task.isNull())
    {
        QDomElement element=task.toElement();
        QString ip=element.attribute("ip");
        QString name=element.attribute("name");
        QString file=element.attribute("file");
        fileList.push_back(std::make_tuple(ip.toStdString(), file.toStdString(), name.toStdString()));
        task=task.nextSibling();
    }
    _serverComm->SetFtpHome(appPath.toStdString());
    return fileList;
}
*/
